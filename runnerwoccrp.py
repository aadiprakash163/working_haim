...
def generate_routefile():
    ...
    with open("data/cross.rou.xml", "w") as routes:
        ...
        <vType id="type2" accel="%f" decel="%f" sigma="0" length="3" width = "2" minGap="0.0" minGapLat = "0.0" maxSpeed="25" guiShape="bus"/>
...
def insert_in_stack(cp,intime,passing_time):
    ...
    if reservation_stack[cp][-1][0] < reservation_stack[cp][-2][1]:
...
def setVelocity(VehId):
    ...
    arrival_time = traci.simulation.getCurrentTime() - 10
    length = traci.vehicle.getLength(VehId)
    # print("vehId: %r    vel: %r    in_time: %r" %(VehId, vel, arrival_time))
    ...
    if routeId=="WtoS" or routeId=="StoE" or routeId == "EtoN" or routeId == "NtoW":
        ...
        if slope1 > slope2: 
            fefs_critical_cp = cp1; cp_dist = totalLength + d1
        else: fefs_critical_cp = cp2; cp_dist = totalLength + d1 + d2
        ...
        while(reservation_stack[fefs_critical_cp][-1][1] + safety_gap*1000)> (dep_time ):
            V = V - 0.01
            trans_time1 = abs((V - vel)/acc)
            trans_time2 = (cp_dist - (abs(V*V - vel*vel)/(2*acc))) / V
            b_time = trans_time1 + trans_time2
        ...
        # V = V_lane
        # cp1_clear=False
        # while cp1_clear==False:# and V > 0.01 :
        #     cp1_time = arrival_time + (trans_time1+(totalLength-abs(V*V-vel*vel)/(2*acc)+d1)/V)*1000
        #     while(reservation_stack[cp1][-1][1] > (cp1_time + safety_gap*1000)):# and V > 0.01:
        #         V=V-0.01
        #         trans_time1 = abs((V-vel)/acc)
        #         trans_time2 = (totalLength-abs(V*V-vel*vel)/(2*acc)+d1)/V
        #         cp1_time = arrival_time + (trans_time1+trans_time2)*1000
        #         if V<0:
        #             print('Velocity crossed 0 at CP1...Exiting')
        #             exit()
                
        #     cp1_clear = True
        #     cp2_time  = cp1_time + (d2/V)*1000
        #     while(reservation_stack[cp2][-1][1] + safety_gap*1000> (cp2_time )):# and V > 0.01:
        #         cp1_clear = False
        #         V=V-0.01
        #         trans_time1 = abs((V-vel)/acc)
        #         trans_time2 = (totalLength-abs((V*V-vel*vel))/(2*acc)+(d1+d2))/V
        #         cp2_time= arrival_time + (trans_time1+trans_time2)*1000
        #         if V<0:
        #             print('Velocity crossed 0 at CP2...Exiting')
        #             exit()
                                

        V_fefs = V

        # print("v_fefs: %r" %V_fefs)
        ...
        V_window = V
        # print("V_window: %r" %V_window)

        V1 = V_lane
        trans_time1 = abs((V1 - vel)/acc)
        trans_time2 = (totalLength  - (abs(V1*V1 - vel*vel)/(2*acc)) + d1) / V1
        b_time = trans_time1 + trans_time2
        dep_time = arrival_time + b_time * 1000.0

        while(reservation_stack[cp1][-1][1] + safety_gap*1000)> (dep_time ):
            V1 = V1 - 0.01
            trans_time1 = abs((V1 - vel)/acc)
            trans_time2 = (totalLength  - (abs(V1*V1 - vel*vel)/(2*acc)) + d1) / V1
            b_time = trans_time1 + trans_time2
        ...
        V2 = V_lane
        trans_time1 = abs((V2 - vel)/acc)
        trans_time2 = (totalLength  - (abs(V2*V2 - vel*vel)/(2*acc)) + d1 + d2) / V2
        b_time = trans_time1 + trans_time2
        dep_time = arrival_time + b_time * 1000.0

        while(reservation_stack[cp2][-1][1] + safety_gap*1000)> (dep_time ):
            V2 = V2 - 0.01
            trans_time1 = abs((V2 - vel)/acc)
            trans_time2 = (totalLength  - (abs(V2*V2 - vel*vel)/(2*acc)) + d1 + d2) / V2
            b_time = trans_time1 + trans_time2
        ...
        V3 = V_lane
        trans_time1 = abs((V3 - vel)/acc)
        trans_time2 = (totalLength  - (abs(V3*V3 - vel*vel)/(2*acc)) + d1 + d2 + d3) / V3
        b_time = trans_time1 + trans_time2
        dep_time = arrival_time + b_time * 1000.0

        while(reservation_stack[cp3][-1][1] + safety_gap*1000)> (dep_time ):
            V3 = V3 - 0.01
            trans_time1 = abs((V3 - vel)/acc)
            trans_time2 = (totalLength  - (abs(V3*V3 - vel*vel)/(2*acc)) + d1 + d2 + d3) / V3
            b_time = trans_time1 + trans_time2
        ...
        V4 = V_lane
        trans_time1 = abs((V4 - vel)/acc)
        trans_time2 = (totalLength  - (abs(V4*V4 - vel*vel)/(2*acc)) + d1 + d2 + d3 + d4) / V4
        b_time = trans_time1 + trans_time2
        dep_time = arrival_time + b_time * 1000.0

        while(reservation_stack[cp4][-1][1] + safety_gap*1000)> (dep_time ):
            V4 = V4 - 0.01
            trans_time1 = abs((V4 - vel)/acc)
            trans_time2 = (totalLength  - (abs(V4*V4 - vel*vel)/(2*acc)) + d1 + d2 + d3 + d4) / V4
            b_time = trans_time1 + trans_time2
        ...
        # if slope1 > slope2: 
        #     max_slope  = slope1; res_critical_cp = cp1; cp_dist = totalLength + d1
        # else:max_slope = slope2; res_critical_cp = cp2; cp_dist = totalLength + d1 + d2

        # if slope3 > max_slope:
        #     max_slope = slope3; res_critical_cp = cp3; cp_dist = totalLength + d1 + d2 + d3

        # if slope4 > max_slope:
        #     max_slope = slope4; res_critical_cp = cp4; cp_dist = totalLength + d1 + d2 + d3 + d4
        ...
        # while(reservation_stack[res_critical_cp][-1][1] + safety_gap*1000 > (dep_time)):
        #     V = V - 0.01
        #     trans_time1 = abs((V - vel)/acc)
        #     trans_time2 = (cp_dist - (abs(V*V - vel*vel)/(2*acc))) / V
        #     b_time = trans_time1 + trans_time2
        ...
        if V_final > vel:
            traci.vehicle.setMaxSpeed(VehId, V_final)
        # else:
            # print("veh id %r has final velocity (%r) less than vel (%r) duration is %r" %(VehId, V_final, vel, time))

        traci.vehicle.slowDown(VehId,V_final,time*1000)
    ...
    # if VehId=='EtoS_9':
    #     print("arrival time: %r" %arrival_time)        
    #     # print("fefs_critical_cp: %r \t v_fefs: %r \n" %(fefs_critical_cp, V_fefs))
    #     # print("res_critical_cp: %r \t v_res: %r \n" %(res_critical_cp, V_reservation))
    #     print('V_final: %r' %V_final)
    #     # print(int(1000*time))
    #     print(reservation_stack[cp3])
